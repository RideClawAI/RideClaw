...
查询支付状态（由 Agent 在 `pay` 返回后自动调用，无需用户触发）。命令会先返回当前状态，然后内部自动轮询直到支付成功或超时。支付成功后会自动创建打车订单。
...
- 首次使用请先运行 `init` 绑定手机号
- `pay` 命令是核心流程：Agent 调用 `pay` 后，必须自动链式执行 `pay-status` → `query-order` → `driver-location` 轮询，全程不需要用户确认
